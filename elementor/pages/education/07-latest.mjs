...
/* Source of truth: dist/education.html, the <section class="sol-latest"> block
   (lines 359-386). Every class, string and attribute below is read from that
   file, not typed from memory. This section carries no HTML comment in source.

   THIS SECTION DEPENDS ON NONE OF THE SEVEN INHERITED BLOCKS, checked rather
   ...
   which needs no `align-self` and carries its own class on both sides, so
   neither block 40 nor block 41 has anything to do here.
   ...
   3. THE STUB LIST IS ONE html() BLOB, for all four of the reasons
      ...
      THIS PAGE'S THREE STUBS ARE ITS OWN and were read off dist/education.html
      rather than carried over: two Research and one Article, where `work`
      carries two Article and one Research. The newest is dated June 24, 2026,
      which is the most recent date anywhere in the solution unit.
   ...
   4. SHAPE C, SITE TWO OF TWO: `.sol-latest__more` TAKES ROUTE A, ONE html()
      ...
      re-derived from this page's markup rather than carried over from either
      sibling, and both landed the same way.

      `.sol-latest__more` (dist/education.html:384) is a bare <p> whose only
      child is `<a class="em-btn em-btn--inverse-outline em-btn--md">`, and
      ...
      {box-shadow:var(--shadow-focus-inverse)}` at 0,2,0, which outranks 0,1,1
      on its own, AND the class is on the real focusable anchor here rather
      than on a wrapper, so the WCAG 2.4.7 failure blocks 40 and 41 repair
      cannot arise on this button at all. Block 41's own selector is qualified
      with `.elementor-widget-button` precisely so it cannot reach an authored
      anchor like this one. Probed at rest, on hover and on real keyboard
      focus anyway; the numbers are in the task report.
      ...
      would take `p|See all education research` out of census() and
      `a|See all education research` out of controlBoxes() at once. Keeping it
      is what makes all 57 census keys achievable on this page.

   5. `id="latest-title"` is authored in the markup, per 01-hero.mjs note 5. */

const HEAD = 'The Latest on Education';
