...
- **Our Solutions is a top-level destination.** In `header-2.html` it is now a
  ...
  that page. The mega-menu header (`header.html`) keeps the old label, because it only
...
The filenames stay `current-2` and `current`, because renaming them would break the
...
signature interaction, so read that first before changing anything in it.
...
`build.mjs` exports `PAGES`, and `test.mjs` imports it, so adding a page to the
...
homepage-specific, so moving it to another page means copying two files and adding
attributes.
...
script fails to load, nothing is hidden, and the page just renders without motion.
...
control bar (`.ctl`) is deliberately **not** sticky, because it would sit on top of the
...
reduced-motion users. The stylesheet alone does not get this right, so ship the
...
only the `[data-dropdown="on"]` gate makes it an overlay, the same
...
`minmax(0,680px)` cap, which lets the growing padding eat directly into the fixed
...
across multiple components, and are **pending a design decision**, and they are
...
2. Enqueue in cascade order. Take the list from the page's own `<head>`, which is
   ...
   `components/components.css`, then **`css/site.css`**, which carries the shared
...
| Layout → Widgets Space | n/a | Zero it. Vertical rhythm here is section padding, not widget gaps |
...
| | `04-stories` | **Loop Grid**, carrying the older `auto-populated` marker |
...
- **Graduated in the paint.** The ::before's background is a `to top left`
  ...
  layers add rather than intersect and the fallback is a solid ink blob, and a test
  pins this.
