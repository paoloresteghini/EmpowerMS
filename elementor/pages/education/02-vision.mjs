...
/* Source of truth: dist/education.html, the <section class="sol-vision"> block
   ...
   3. THIS SECTION INHERITS BLOCK 36, AND THE INHERITANCE IS CONDITIONAL ON
      ...
      `safety` and `work`. An html() blob for the whole block, or a container
      around any of the three, would put a different element under that child
      combinator and the block would do nothing. Three text() widgets, direct
      children, and nothing between them.
   ...
   4. `data-reveal` IS AUTHORED IN THE MARKUP IN THIS SECTION, NOT ON THE
      ...
      <p>, which is where dist/education.html puts it, the reveal layer behaves
      exactly as it does in the static build.

      TWO OF THE THREE CHILDREN CARRY IT AND THE THIRD DOES NOT, read off
      dist/education.html:185-187 rather than assumed from the pattern: the
      <h2> and `.sol-vision__lede` carry `data-reveal="rise"` and
      `.sol-vision__body` carries nothing at all. `safety` and `work` are the
      same, and the third widget still needs `display:contents` for its
      `grid-column:2`, so block 36 has to reach all three regardless of which
      ones animate.
   ...
   7. THE SOURCE COMMENT (179-181) IS THIS PAGE'S OWN AND IS NOT `work`'s. Its
      third line reads "listed underneath it" where dist/work.html:181 reads
      "for people and businesses listed underneath it". Reproduced as this file
      has it, which is the whole reason every string here is read off this
      page's markup rather than copied from the fill before it. */
...
const HEAD = 'What is Education Freedom?';
const LEDE = 'A quality education should open doors to opportunity.';
const BODY = 'That means empowering parents to make the best decisions for their children, giving '
  + 'educators the freedom to meet students’ needs, and creating more pathways for every child to '
