...
/* Source of truth: dist/education.html, the <section class="sol-hero"> block
   ...
   from elementor/pages/work/01-hero.mjs or elementor/pages/safety/01-hero.mjs:
   the three sections are the same SHAPE and none of the same words.

   THIS PAGE IS A FILL, NOT A CONVERSION, AND THE LAST OF THREE.
   css/solution.css is shared by `safety`, `work` and `education`; `safety`
   converted it first (Task 17) and `work` filled it (Task 18). Seven bridge
   blocks now reach this template, and every one of them is keyed on a class the
   shared stylesheet or the shared button component gives all three pages. That
   inheritance is conditional on this page being built in the same shape, which
   is why each module below states which of the seven its own section depends
   ...
   1. CONTAINERS ARE 'full' THROUGHOUT. The build supplies its own
      `.em-container` measure (tokens/base.css:18) and Elementor's boxed
      container would insert a second, narrower one inside it. Same choice
      every earlier page.mjs and section module in this build makes.
   ...
   2. `.em-container` IS ITS OWN DIV, not merged with `.sol-hero__inner`,
      ...
      div some other pages use. Read from dist/education.html rather than
      copied.
   ...
   4. THE <em> IS PART OF THE HEADING'S OWN MARKUP STRING. css/solution.css:76
      ...
      is what puts the orange half of the sentence on its own line, which on
      this page is "Opportunity to Succeed".

   5. `id="hero-title"` IS AUTHORED IN THE MARKUP, not passed through
      `_element_id` or `_attributes`. The id belongs to the <h1> itself (the
      section's `aria-labelledby` names it), and a text() widget's markup
   ...
   7. THE SOURCE COMMENT ABOVE THE SECTION (dist/education.html:168-170) is
      ...
      widget's own markup, which reaches the page unaltered. THIS PAGE'S
      COMMENT IS BYTE FOR BYTE `work`'s, checked against both files rather than
      assumed: both say "The template's whole idea", where `safety` says "This
      variation's whole idea". */
...
const EYEBROW = 'Quality Education';
const HEAD = 'Every Child Deserves the <em>Opportunity to Succeed</em>';
